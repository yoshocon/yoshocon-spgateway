...
class MPG
{
    ...
    public function __construct()
    {
        ...
        {
            $this->apiUrl['MPG_API'] = 'https://ccore.spgateway.com/MPG/mpg_gateway';
     ...
     */
    public function generate($amount, $email, $itemDesc, array $params = [])
    {
        try
        {
     ...
     * @return array|bool
     */
    private function formValidator($amount, $itemDesc, $email, $params)
    {
        if (isset($params['LangType']))
        {
            if (!in_array($params['LangType'], ['en', 'zh-tw']))
            {
                return $this->errorMessage('LangType', '英文版參數為 en，繁體中文版參數為 zh-tw');
            }
        }

        if (!is_numeric($amount)) { return $this->errorMessage('Amt', '必須為大於0的整數'); }

        if (strlen($itemDesc) > 50) { return $this->errorMessage('Amt', '必須小於50字'); }
        ...
        {
            if (date_parse_from_format('Ymd', $params['ExpireDate'])['error_count'] > 0)
            {
                return $this->errorMessage('ExpireDate', '格式需為Ymd，如:20170101');
            }

            if (strtotime($params['ExpireDate']) < strtotime('today') || strtotime('-180 days', strtotime($params['ExpireDate'])) > strtotime('today'))
            {
        ...
        {
            if (!filter_var($params['ReturnURL'], FILTER_VALIDATE_URL)) { return $this->errorMessage('ReturnURL', '必須為合法的Url'); }
        }

        if (isset($params['NotifyURL']))
        {
            if (!filter_var($params['NotifyURL'], FILTER_VALIDATE_URL)) { return $this->errorMessage('NotifyURL', '必須為合法的Url'); }
        }

        if (isset($params['CustomerURL']))
        {
            if (!filter_var($params['CustomerURL'], FILTER_VALIDATE_URL)) { return $this->errorMessage('CustomerURL', '必須為合法的Url'); }
        }

        if (isset($params['ClientBackURL']))
        {
            if (!filter_var($params['ClientBackURL'], FILTER_VALIDATE_URL)) { return $this->errorMessage('ClientBackURL', '必須為合法的Url'); }
        }

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { return $this->errorMessage('Email', '必須為合法的Email'); }

        if (isset($params['EmailModify']))
        {
            if (!in_array($params['EmailModify'], [1, 0])) { return $this->errorMessage('EmailModify', '必須為0或1'); }
        }

        if (isset($params['LoginType']))
        {
            if (!in_array($params['LoginType'], [1, 0])) { return $this->errorMessage('LoginType', '必須為0或1'); }
        }

        if (isset($params['OrderComment']))
        {
            if (strlen($params['OrderComment']) > 300) { return $this->errorMessage('OrderComment', '必須小於300字'); }
        }

        if (isset($params['CREDIT']))
        {
            if (!in_array($params['CREDIT'], [1, 0])) { return $this->errorMessage('CREDIT', '必須為0或1'); }
        }

        if (isset($params['CreditRed']))
        {
            if (!in_array($params['CreditRed'], [1, 0])) { return $this->errorMessage('CreditRed', '必須為0或1'); }
        }
        ...
        if (isset($params['InstFlag']))
        {
            ...
            {
                if (!in_array($val, [1, 3, 6, 12, 18, 24])) { return $this->errorMessage('InstFlag', '期數必須為3、6、12、18、24'); }
            });
        }

        if (isset($params['UNIONPAY']))
        {
            if (!in_array($params['UNIONPAY'], [1, 0])) { return $this->errorMessage('UNIONPAY', '必須為0或1'); }
        }

        if (isset($params['WEBATM']))
        {
            if (!in_array($params['WEBATM'], [1, 0])) { return $this->errorMessage('WEBATM', '必須為0或1'); }
        }
        ...
        {
            if (!in_array($params['VACC'], [1, 0])) { return $this->errorMessage('VACC', '必須為0或1'); }
        }

        if (isset($params['CVS']))
        {
            if (!in_array($params['CVS'], [1, 0])) { return $this->errorMessage('CVS', '必須為0或1'); }
        }

        if (isset($params['BARCODE']))
        {
            if (!in_array($params['BARCODE'], [1, 0])) { return $this->errorMessage('BARCODE', '必須為0或1'); }
        }
    ...
    private function createMpgAesEncrypt($parameter = "")
    {
        ...
        if (!empty($parameter)) { $return_str = http_build_query($parameter); }
    ...
    public function search($orderNo, $amount)
    {
        ...
        $postData['CheckValue'] = $this->generateTradeInfoCheckValue($orderNo, $amount);
        $res = $this->helpers->sendPostRequest($this->apiUrl['QUERY_TRADE_INFO_API'], $postData);
    ...
    private function parseCommodities($commodities = [])
    {
        ...
        foreach ($commodities as $index => $commodity)
        {
            ...
            foreach ($commodity as $key => $value) { $newCommodity[$key . ($index + 1)] = $value; }
